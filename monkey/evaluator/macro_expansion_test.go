...
// TestDefineMacros はマクロ定義がASTから抽出されて環境に格納されることをテストする。
// マクロ定義文はASTから削除され、通常のlet文はそのまま残る。
// 付録で追加。
...
// TestExpandMacros はマクロが正しく展開されることをテストする。
// マクロ呼び出しが展開後のASTに正しく置換されることを検証する。
// 付録で追加。
...
// testParseProgram は入力文字列をパースしてASTのProgramノードを返すヘルパー。
